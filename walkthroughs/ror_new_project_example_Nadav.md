...
#### Step-by-Step setting up a new Ruby on Rails project with postgresql databases:
	...
	(5)	Terminal> 	bundle install
		...
		// Consequently, the gems under the'production' group in the Gemfile will be installed Heroku during deployment 
		// (in other words, gems in this group will not be installed locally - note that this install must be done on the FIRST bundle install otherwise won't work.
		    after the first time, you can use 'buntle install' as usual).
